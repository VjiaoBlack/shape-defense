    ...
    case 1:
      if (m_targetEnt) {
        ...
        if (distSq > 60.0 * 60.0) {
          ...
          m_ent->get<TdECSPhysicsComponent>()->m_vx = entX / sqrt(distSq);
          m_ent->get<TdECSPhysicsComponent>()->m_vy = entY / sqrt(distSq);
        } else {
          // stop
        ...
        for (auto& ent : game->m_entitySystem->m_entities) {
          ...
          if (ent->has<TdECSShooterComponent>()) {
            ...
            double myX = m_ent->get<TdECSPositionComponent>()->m_x;
            double myY = m_ent->get<TdECSPositionComponent>()->m_x;
