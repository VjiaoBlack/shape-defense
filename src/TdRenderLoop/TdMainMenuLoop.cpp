...
TrMainMenuLoop::TrMainMenuLoop(TdGame *game) {
  ...
  m_GUISystem = std::make_unique<TdGUISystem>();
  ...
  m_GUIMenu = TdGUIEntity::addVerticalMenu(game, m_GUISystem.get(), r,
                                           std::move(mylabels));
...
TdRenderLoop *TrMainMenuLoop::update(TdGame *game) {
  if (m_GUIMenu->get<TdGUIContainerComponent>()
          ->m_buttons[0]
          ->get<TdGUIClickableComponent>()
          ->m_activated) {
    auto gameLoop = std::make_shared<TdGameLoop>(game);
...
void TrMainMenuLoop::render(TdGame *game) {
